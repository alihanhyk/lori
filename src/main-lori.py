...
params['r'] = .001 * jax.random.normal(subkey, shape=(2,2))
subkey, key = jax.random.split(key)
params['r_max'] = .001 * jax.random.normal(subkey)
subkey, key = jax.random.split(key)
params['eps0'] = .001 * jax.random.normal(subkey)
subkey, key = jax.random.split(key)
params['eps1'] = .001 * jax.random.normal(subkey)
...
with open('res/res-lori-k{}.obj'.format(args.key), 'wb') as f:
