...
def pref2(del0, del1):
    ...
    prob = prob + (1-1/(1+np.exp(alp0*(eps0-del0)))-1/(1+np.exp(alp0*(eps0+del0)))) * 1/(1+np.exp(alp1*(eps1-del1)))
    prob = prob + (1-1/(1+np.exp(alp0*(eps0-del0)))-1/(1+np.exp(alp0*(eps0+del0)))) * (1-1/(1+np.exp(alp1*(eps1-del1)))-1/(1+np.exp(alp1*(eps1+del1)))) / 2
    return prob

@jax.jit
def pref2_long(del0, del1, eps0, eps1):
    prob = 1/(1+np.exp(eps0-del0))
    prob = prob + (1-1/(1+np.exp(eps0-del0))-1/(1+np.exp(eps0+del0))) * 1/(1+np.exp(eps1-del1))
    prob = prob + (1-1/(1+np.exp(eps0-del0))-1/(1+np.exp(eps0+del0))) * (1-1/(1+np.exp(eps1-del1))-1/(1+np.exp(eps1+del1))) / 2
    return prob
...
def pref2_long1_equiv(del0, del1, eps0, eps1):
    return (1-1/(1+np.exp(eps0-del0))-1/(1+np.exp(eps0+del0))) * (1-1/(1+np.exp(eps1-del1))-1/(1+np.exp(eps1+del1)))

@jax.jit
def pref2_long2(del0, del1, del2, eps0, eps1, eps2):
    prob = 1/(1+np.exp(eps0-del0))
    prob = prob + (1-1/(1+np.exp(eps0-del0))-1/(1+np.exp(eps0+del0))) * 1/(1+np.exp(eps1-del1))
    prob = prob + (1-1/(1+np.exp(eps0-del0))-1/(1+np.exp(eps0+del0))) * (1-1/(1+np.exp(eps1-del1))-1/(1+np.exp(eps1+del1))) * 1/(1+np.exp(eps2-del2))
    return prob

@jax.jit
def pref2_long2_equiv(del0, del1, del2, eps0, eps1, eps2):
    return (1-1/(1+np.exp(eps0-del0))-1/(1+np.exp(eps0+del0))) * (1-1/(1+np.exp(eps1-del1))-1/(1+np.exp(eps1+del1))) * (1-1/(1+np.exp(eps2-del2))-1/(1+np.exp(eps2+del2)))
...
state_vals = np.array([[[x0, x1]
        for x1 in np.linspace(state_x1_min, state_x1_max, state_n)]
...
_get_T1 = lambda x, u: np.exp(_get_T0(x, u, state_vals)-logsumexp(_get_T0(x, u, state_vals)))
...
def _interp(x):
    i0 = np.maximum(0, np.minimum(1, (x[0]-state_x0_min)/(state_x0_max-state_x0_min)))
    i0 = np.round(i0 * (state_n-1)).astype(int)
    i1 = np.maximum(0, np.minimum(1, (x[1]-state_x1_min)/(state_x1_max-state_x1_min)))
...
def _solve(arg0, arg1):
    ...
    v = lex_maxmap(q)
    q = np.minimum(R_max/(1-gmm), R + np.minimum(R_max/(1-gmm), gmm * np.tensordot(T, v, axes=1)))
...
state1_n = 25
state1_vals = np.array([[[x0, x1]
        for x1 in np.linspace(state_x1_min, state_x1_max, state1_n)]
...
_get_T10 = lambda x, u: np.exp(_get_T0(x, u, state1_vals)-logsumexp(_get_T0(x, u, state1_vals)))
