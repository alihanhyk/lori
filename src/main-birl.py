...
def sample(_r, like, key):
    (_r, like), (_rs, conds) = jax.lax.scan(_sample, (_r, like), jax.random.split(key, 1000))
    return _r, like, _rs, conds.sum() / 1000

subkey, key = jax.random.split(key)
_r = .01 * jax.random.normal(subkey, shape=(2,))
like = likelihood(_r)
...
with open('res/res-birl-k{}.obj'.format(args.key), 'wb') as f:
    dill.dump(r, f)
