...
q_opt = solve(0.95, R_true, R_true_max, T)
...
subkey, key = jax.random.split(key)
pref_test_is, pref_test_js = rank_batch(
    data_xs, data_us, jax.random.split(subkey, 1000)
)

with open(f"data/data-k{args.key}.obj", "wb") as f:
