...
from simu import pref2, r_true, r_true_max
...
for k in range(5):
    for f1 in fs:
        for f2 in fs:
            with open(f1 + f"-k{k}.obj", "rb") as f:
                traj1_xs, traj1_us = dill.load(f)
            with open(f2 + f"-k{k}.obj", "rb") as f:
...
print(
    "           {}".format(
        " ".join(["{:<10}               ".format(label) for label in labels])
    )
)
for f1, label1 in zip(fs, labels):
    ...
    print(
        "{:<8} : {}".format(
            label1,
            " ".join(
                ["{:<10} ({:<12})".format(r0, r1) for r0, r1 in zip(res_mean, res_std)]
            ),
        )
    )
